...
const Home = () => {
  ...
  return (
    <div className="bg-transparent h-full md:py-3 py-8 md:my-2  mx-auto md:flex-col flex w-full md:p-0 ">
      <div className=" md:grid grid-cols-[58.6fr_34.5fr] grid-rows-1 md:gap-4  h-full w-full ">
        ...
        <div className="md:grid grid-rows-[56.9fr_43.1fr] grid-cols-9 md:gap-4 h-full flex-1 col-start-1 ">
            ...
            <div   className="ContactCard bg-orange-300 rounded-3xl border hover:text-shadow-glow hover:shadow-[0_0_30px_rgba(255,255,255,0.6)] md:row-start-2 md:col-span-5 md:p-5 p-10 h-full w-full md:relative md:overflow-hidden flex flex-col">
              <div className="font-light md:py-0 py-5 md:absolute md:left-10 md:top-11 leading-tight md:text-[clamp(1rem,1.1vw,1.2vw)]">
                <p>Have some</p>
                <span >questions?</span>
              </div>        
              <FancyButton to="/contact" className="bg-white absolute bottom-20 md:left-35 left-75  border-black border-1.5 rounded-full p-0 w-14 h-14 flex items-center justify-center"
              ...
              />
              <span className="font-medium leading-tight text-[clamp(1.5rem,3.8rem,5rem)] md:absolute md:bottom-10">Contact me.</span>
            </div>
        ...
        <div className="md:grid grid-rows-[46.9fr_42.7fr_12.2fr] grid-cols-1 col-start-2 gap-4 h-full flex-1">
            ...
            <FancyButton text="INSTAGRAM" href={"https://www.instagram.com/kevin_babu.in/"} className="border border-black text-sm px-9 bg-white rounded-full md:p-0 p-5" to="/projects" />
